...
const GAME_DATA = {
  imagePath: "/BookPics/pabg.png", // Single image for entire game (2:3 aspect ratio, 1024x1536px) - served from public folder
  rounds: [
    {
      roundNumber: 1,
...
const Book2 = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/conlimit?bookId=book2`)
      .then((res) => res.json())
      .then((data) => setConLimit(data.conLimit ?? 10))
      .catch(() => setConLimit(10));

    fetch(`${API_BASE_URL}/api/bookdata?bookId=book2`)
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(`${API_BASE_URL}/api/bookmeta?bookId=book2`)
      .then((res) => res.json())
      .then((data) => {
